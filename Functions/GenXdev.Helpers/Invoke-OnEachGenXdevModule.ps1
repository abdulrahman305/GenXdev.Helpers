...
function Invoke-OnEachGenXdevModule {
    ...
    process {
        ...
        $allGenXdevModules = @(GenXdev.FileSystem\Find-Item "$modulesPath\GenXdev*\1.252.2025\GenXdev*.psm1" -PassThru -NoRecurse | Microsoft.PowerShell.Core\ForEach-Object { [IO.Path]::GetFileNameWithoutExtension($_.FullName) }) +
            @("GenXdev.Scripts")
        ...
        foreach ($nextModule in $uniqueModulesNames) {
            ...
            function processModule {
                ...
                $licenseFilePath = (
                    "$($ModuleObj.FullName)\1.252.2025\LICENSE")

                # construct expected readme file path for published modules
                $readmeFilePath = (
                    "$($ModuleObj.FullName)\1.252.2025\README.md")
