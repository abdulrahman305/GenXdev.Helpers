...
function Invoke-OnEachGenXdevModule {
    ...
    begin {
        ...
        $allGenXdevModules = @(GenXdev.FileSystem\Find-Item "$modulesPath\GenXdev*\2.1.2025\GenXdev*.psm1" -PassThru -NoRecurse | Microsoft.PowerShell.Core\ForEach-Object { [IO.Path]::GetFileNameWithoutExtension($_.FullName) }) +
            @("GenXdev.Scripts")
  ...
  process {
        ...
        foreach ($nextModule in $uniqueModulesNames) {
            ...
            function processModule {
                ...
                $licenseFilePath = (
                    "$($ModuleObj.FullName)\2.1.2025\LICENSE")

                # construct expected readme file path for published modules
                $readmeFilePath = (
                    "$($ModuleObj.FullName)\2.1.2025\README.md")
